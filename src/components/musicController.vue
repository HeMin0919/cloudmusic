  ...
  <div>
        ...
        <div class="player-main-main" @click="toMusicMsg(musicId)">
          <div>
            ...
            <div class="artistsname">
              <span v-for="(item, index) in artists" :key="index">{{item.name}}<span v-if="index < (artists.length - 1)">/</span></span>
            </div>
          </div>
        </div>
    ...
    <mu-bottom-sheet :open="bottomSheet" @close="closeBottomSheet" :sheetClass="{maxHeight: bottomSheet}">
      <mu-list :value="musicId" @itemClick="closeBottomSheet">
        ...
        <mu-list-item v-for="(item, index) of musicUrlList" :key="item.id"
  ...
  export default {
    ...
    methods: {
      ...
      next () {
        ...
        this.$http.get(`http://localhost:5000/music/url?id=${id}`)
          .then((res) => {
            // 下一首歌曲的url
