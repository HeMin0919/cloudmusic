<template>
  <div>
    ...
    <header @click="playMusic()" class="comment-header" v-if="commentMsg">
      ...
      <span class="icon-Rightclick"></span>
    </header>
  ...
  export default {
    ...
    computed: {
      ...
      commentMsg () {
        ...
        if (!this.$store.state.commentHeader.artists) {
          this.$http.get(`http://localhost:5000/song/detail?ids=${id}`)
            .then((res) => {
    ...
    methods: {
      ...
      fetchData (type) {
        ...
        this.loading = true
        var id = this.$route.params.id;
        this.$http.get(`http://localhost:5000/comment/${type}?id=${id}&limit=10&offset=${offset}`)
          .then((res) => {
